...
from app.preset_store import PresetStore
from app.services.web_sources_service import WebSourcesService
from app.settings_store import default_web_sources_registry
...
from engine.analyze.gif_scan import GifScanInput, estimate_gif_palette_stress_stub
from engine.analyze.quality_scan import QualityScanInput, scan_quality
from engine.analyze.recommend import RecommendationInput, build_recommendations
from engine.batch.batch_runner import BatchRunner, BatchRunnerConfig, BatchWorkItem, BatchRunReport
from engine.classify.classifier import classify_asset
from engine.export.exporters import ExportRequest, ExportResult, export_image
from engine.export.size_predictor import ExportPredictorInput, ExportPredictorResult, predict_export_size
from engine.export.naming import safe_stem as export_safe_stem, ensure_unique_path
from engine.ingest.local_ingest import LocalIngestResult, build_local_ingest_queue
from engine.ingest.url_ingest import (
    DownloadGuards,
...
from engine.ingest.webpage_scan import (
    WebpageScanFilters,
    WebpageScanResult,
...
from engine.process.heavy_queue import HeavyQueueEngine
from engine.process.bounds import clamp_edit_state_for_mode
from engine.process.light_steps import LightProcessError, LightStepUnavailable, apply_light_processing
from engine.process.presets_apply import (
    PresetApplyError,
    ViewEditStates,
    apply_preset_stack,
...
class ImageEngineUIController:
    ...
    def apply_light_pipeline(self, asset: AssetRecord) -> bool:
        ...
        """
        source = asset.cache_path or asset.source_uri
        if not source:
            return False
        try:
        ...
        # Resolve derived output paths inside the app cache.
        base_cache = (self.app_paths.cache if self.app_paths is not None else Path(".") / "_derived_cache")
        derived_dir = base_cache / "derived" / asset.id
        current_out = derived_dir / "current.png"
        final_out = derived_dir / "final.png"

        # Apply-target rules (sync mirrors output).
        target = getattr(asset.edit_state, "apply_target", None)
        target_value = getattr(target, "value", "both")

        try:
            if bool(asset.edit_state.sync_current_final) or target_value == "both":
                result = apply_light_processing(source_path=source, output_path=final_out, settings=asset.edit_state.settings)
                asset.derived_final_path = str(result.output_path)
                # Mirror final -> current for now (keeps them consistent when sync/both).
                ...
                if final_out.exists():
                    try:
                        ...
                        # Fall back to final preview path if mirror copy fails.
                        mirrored_current = final_out
                asset.derived_current_path = str(mirrored_current)
                asset.dimensions_current = result.size
                asset.dimensions_final = result.size
                return True

            if target_value == "final":
                result = apply_light_processing(source_path=source, output_path=final_out, settings=asset.edit_state.settings)
                asset.derived_final_path = str(result.output_path)
                asset.dimensions_final = result.size
                return True
            ...
            # current only
            result = apply_light_processing(source_path=source, output_path=current_out, settings=asset.edit_state.settings)
            asset.derived_current_path = str(result.output_path)
            asset.dimensions_current = result.size
            return True
    ...
    def import_url_source(
        ...
        *,
        ...
        if stream_preview:
            ...
            try:
                preview = stream_preview_mode_stub(
                    effective_url,
        ...
        try:
            ...
            # If user pasted a webpage URL (or a likely link that returns HTML),
            # auto-resolve the first image so URL ingest remains user-friendly.
            fallback_url: str | None = None
            if allow_webpage_fallback and normalize_ext(effective_url) not in ALLOWED_IMAGE_EXTS_DEFAULT:
    ...
    def apply_heavy_queue(self, asset: AssetRecord) -> list[HeavyJobSpec]:
        ...
        completed = engine.run_all(progress_steps=4, step_delay_seconds=0.0)
        asset.edit_state.queued_heavy_jobs = engine.list_jobs()
        return completed
    ...
    def format_prediction_text(self, asset: AssetRecord) -> str:
        ...
        return (
            f"Predicted: {pred.predicted_format.upper()} "
            f"{pred.predicted_bytes:,} bytes ({predictor.compression_efficiency_rating})"
        )
        ...
        *,
        ...
        return export_image(
            ExportRequest(
                ...
                source_path=self._select_export_source_path(asset),
                width=max(1, width),
                height=max(1, height),
                export_settings=asset.edit_state.settings.export,
                asset_id=asset.id,
    ...
    def _select_export_source_path(self, asset: AssetRecord) -> str | None:
        ...
        Default: prefer derived_final_path if present, else cache_path.
        If exporting GIF from an animated source, prefer the original cache_path so frames can be preserved.
        """
        ...
        fmt = getattr(export_settings, "format", None)
        try:
            from engine.models import ExportFormat as _ExportFormat
            if fmt in {_ExportFormat.GIF, _ExportFormat.AUTO} and bool(getattr(getattr(asset, "capabilities", None), "is_animated", False)):
                return str(cache) if cache else (str(derived) if derived else None)
        except Exception:
            pass

        return str(derived) if derived else (str(cache) if cache else None)
    ...
    def _analyze_asset_profile(self, asset: AssetRecord) -> None:
        ...
        if asset.format is AssetFormat.GIF and asset.capabilities.is_animated:
            analysis.gif_palette_stress = estimate_gif_palette_stress_stub(
                GifScanInput(
                    frame_count=8,
