...
from engine.export.naming import safe_stem as export_safe_stem, render_name_template, ensure_unique_path
...
from engine.process.pipeline import PipelinePhase, ProcessingPlan, ProcessingStep, build_processing_plan
from engine.process.presets_apply import ViewEditStates, apply_preset_stack
...
class BatchRunner:
    ...
    def _run_light_pipeline(self, asset: AssetRecord) -> None:
        ...
        if self.config.derived_cache_dir is None:
            return
        ...
        try:
            base = Path(self.config.derived_cache_dir)
        except Exception:
            return

        out_dir = base / "derived" / asset.id
        out_path = out_dir / "final.png"

        try:
            result = apply_light_processing(source_path=source, output_path=out_path, settings=asset.edit_state.settings)
    ...
    def _select_export_source_path(self, asset: AssetRecord) -> str | None:
        ...
        Prefer derived_final_path for processed outputs, but preserve GIF frames when exporting GIF from an animated source.
        """
