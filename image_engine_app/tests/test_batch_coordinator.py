...
from image_engine_app.engine.process.heavy_queue import HeavyQueueEngine  # noqa: E402
...
class BatchCoordinatorSelectionTests(unittest.TestCase):
    class _FakeController:
        def __init__(self) -> None:
            ...
            self._batch_workflow = BatchWorkflowService(
                ...
                preset_workflow=self,  # type: ignore[arg-type]
        ...
        def apply_named(
            ...
            *,
            ...
            return self.apply_named_preset(
                asset,
                preset_name,
                refresh_final=refresh_final,
            )

        def prepare_batch_assets(self, **kwargs):  # noqa: ANN003
            return self._batch_workflow.prepare_assets(**kwargs)
    ...
    def test_on_run_requested_can_copy_active_controls(self) -> None:
        ...
        self.assertIn("assets", captured)
        batch_assets = captured["assets"]
        self.assertEqual([asset.id for asset in batch_assets], ["asset-b"])
    ...
    def test_on_run_requested_skips_incompatible_selected_preset_items(self) -> None:
        ...
        class MixedController(self._FakeController):
            def apply_named_preset(
                self,
                asset,
                preset_name: str,
                *,
                ...
                if str(getattr(asset, "id", "")) == "asset-b":
                    raise PresetApplyError("not compatible")
                return super().apply_named_preset(
                    asset,
                    preset_name,
                    refresh_final=refresh_final,
                )

        window.controller = MixedController()
        coordinator = BatchCoordinator(window)
        captured: dict[str, object] = {}
        ...
        self.assertIn("assets", captured)
        self.assertEqual([asset.id for asset in captured["assets"]], ["asset-a", "asset-b"])
        self.assertIn(("asset-a", "Photo Recover"), window.controller.applied_presets)
        self.assertTrue(any("skipped 1 incompatible" in message for message in window.status_messages))

    def test_on_run_requested_can_apply_background_override(self) -> None:
        window = self._FakeWindow()
        target = window._workspace_assets[1]
        target.edit_state.settings.alpha.background_removal_mode = "off"
        target.edit_state.settings.alpha.remove_white_bg = False
        ...
        self.assertEqual([asset.id for asset in batch_assets], ["asset-b"])
        self.assertIsNot(batch_assets[0], target)
        self.assertEqual("black", batch_assets[0].edit_state.settings.alpha.background_removal_mode)
        self.assertEqual("off", target.edit_state.settings.alpha.background_removal_mode)
        self.assertFalse(target.edit_state.settings.alpha.remove_white_bg)
