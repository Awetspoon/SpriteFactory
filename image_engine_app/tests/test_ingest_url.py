...
from image_engine_app.engine.ingest.url_ingest import (  # noqa: E402
    DownloadGuards,
    FileSizeGuardError,
    MimeValidationError,
...
class UrlIngestTests(unittest.TestCase):
    ...
    def test_download_url_to_cache_uses_stable_cache_key_when_download_url_differs(self) -> None:
        data = _fake_png(18, 12, payload=b"keyed")
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            asset = download_url_to_cache(
                "https://cdn.example.com/generated/path/sprite.png?cb=123",
                temp_dir,
                cache_key_url="https://example.com/wiki/File:Sprite.png",
                opener=opener,
            )

            expected = "df291762a4666a36d08c878bb67a310cb5b291f2ab57e8ecf61ecc595209ca7c.png"
            self.assertEqual(expected, asset.cache_path.name)
    ...
    def test_download_uses_no_proxy_fallback_on_winerror_10013(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            with patch("image_engine_app.engine.ingest.url_ingest.urlopen", primary_urlopen), patch(
                "image_engine_app.engine.ingest.url_ingest.build_opener",
    ...
    def test_stream_preview_mode_rejects_signature_mismatch(self) -> None:
        ...
        with self.assertRaises(SignatureValidationError):
            stream_preview_mode("https://example.com/bad.png", opener=opener)
    ...
    def test_stream_preview_mode_rejects_mime_mismatch(self) -> None:
        ...
        with self.assertRaises(MimeValidationError):
            stream_preview_mode("https://example.com/wrong-mime", opener=opener)
