...
from image_engine_app.engine.models import AssetFormat, AssetRecord, SourceType


class WebSourcesNetworkTests(unittest.TestCase):
    @staticmethod
    def _downloader(
        *,
        app_paths=None,
        scan_webpage_images=lambda *_args, **_kwargs: None,
        import_url_source=lambda *_args, **_kwargs: None,
    ) -> WebSourcesDownloader:
        return WebSourcesDownloader(
            app_paths=app_paths,
            scan_webpage_images=scan_webpage_images,
            import_url_source=import_url_source,
            import_cached_files=lambda **_kwargs: ImportResult(),
        )
    ...
    def _scanner(
        ...
        *,
        ...
        downloader = self._downloader(scan_webpage_images=scan_webpage_images)
        return WebSourcesScanner(
            scan_webpage_images=scan_webpage_images,
            canonicalize_download_url=downloader.canonicalize_download_url,
            resolve_web_item_name=downloader.resolve_web_item_name,
            extract_archive_urls=downloader.extract_archive_urls,
        )

    def test_discover_links_returns_same_site_category_pages(self) -> None:
        html = """
        ...
        service = self._scanner()

        links = service.discover_links(
    ...
    def test_scan_pages_merges_and_dedupes_results(self) -> None:
        ...
        def scan_page(area_url, **_kwargs):  # noqa: ANN001
        ...
        service.scan_page = scan_page  # type: ignore[method-assign]
    ...
    def test_resolve_download_url_prefers_file_specific_candidate_over_logo(self) -> None:
        ...
        service = self._downloader(scan_webpage_images=scan_webpage_images)
    ...
    def test_download_items_uses_resolved_media_url_and_stable_canonical_cache_key(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            def import_url_source(url, **kwargs):  # noqa: ANN001
                ...
                return ImportResult(entries=[ImportedAsset(asset=asset, source=url)])

            service = self._downloader(
                app_paths=paths,
                scan_webpage_images=scan_webpage_images,
                import_url_source=import_url_source,
            )
    ...
    def test_download_items_keeps_going_when_media_resolution_fails(self) -> None:
        ...
        service = self._downloader(import_url_source=import_url_source)
