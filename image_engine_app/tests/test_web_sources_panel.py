...
from app.web_sources_models import ImportTarget  # noqa: E402
from ui.main_window.web_sources_panel import WebSourcesPanel  # noqa: E402
...
class WebSourcesPanelTests(unittest.TestCase):
    ...
    def test_jpg_filter_is_available_and_enabled_by_default(self) -> None:
        ...
        try:
            ...
            if owns_app and app is not None:
                app.quit()
    def test_import_target_labels_are_generic(self) -> None:
        app, owns_app, panel = self._setup_panel()
        try:
            labels = [panel._target.itemText(i) for i in range(panel._target.count())]
            data_values = [panel._target.itemData(i) for i in range(panel._target.count())]

            self.assertEqual(["Main", "Variant", "Animated", "Items"], labels)
            self.assertEqual(
                [ImportTarget.NORMAL, ImportTarget.SHINY, ImportTarget.ANIMATED, ImportTarget.ITEMS],
                data_values,
            )
        finally:
            panel.close()
