...
from engine.ingest.webpage_scan import (  # noqa: E402
    WebpageScanCancelledError,
    WebpageScanFilters,
    depth_scan_webpages_stub,
...
class WebpageScanTests(unittest.TestCase):
    ...
    def test_fetch_html_retries_without_proxy_on_winerror_10013(self) -> None:
        ...
        with patch("engine.ingest.webpage_scan.urlopen", primary_urlopen), patch(
            "engine.ingest.webpage_scan.build_opener",
    ...
    def test_depth_scan_stub_routes_to_scan_function(self) -> None:
        def opener(request, timeout=0):  # noqa: ANN001
            _ = (request, timeout)
            return _FakeResponse(HTML_SAMPLE.encode("utf-8"))

        results = depth_scan_webpages_stub(
            ["https://example.com/gallery/page.html"],
            max_depth=1,
            opener=opener,
        )
