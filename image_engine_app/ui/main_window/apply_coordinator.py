...
class ApplyCoordinator:
    ...
    def on_preset_requested(self, preset_name: str) -> None:
        ...
        # Auto-apply light pipeline for instant preview updates when enabled.
        if bool(asset.edit_state.auto_apply_light):
            try:
    ...
    def on_global_reset_requested(self) -> None:
        ...
        try:
            ...
            self._window._refresh_export_prediction()
            self._window._status("Reset current asset to default settings")
        except Exception as exc:
