...
class BatchCoordinator:
    ...
    def on_run_requested(self, options_obj: object) -> None:
        ...
        background_override = (
            normalize_background_removal_mode(background_override_raw).value
            if background_override_raw not in (None, "")
            else None
        )
        ...
        try:
            prep = self._window.controller.prepare_batch_assets(
            ...
            self._window._status(f"Batch run skipped: preparation failed ({exc})")
            return
        batch_assets = prep.assets
        ...
        if prep.skipped_preset_count > 0:
            self._window._status(
                f"Batch preset applied to {prep.applied_preset_count} item(s); skipped {prep.skipped_preset_count} incompatible item(s)"
            )
        ...
        self._window._status(f"Batch run started ({len(batch_assets)} item(s))")
    ...
    def on_run_finished(self, report: object) -> None:
        ...
        skipped = int(getattr(report, "skipped_count", 0))
        LOGGER.info("Batch run finished: processed=%s failed=%s skipped=%s cancelled=%s", processed, failed, skipped, cancelled)
        if cancelled:
            self._window._status(f"Batch cancelled: processed {processed}, failed {failed}, skipped {skipped}")
        else:
            self._window._status(f"Batch complete: processed {processed}, failed {failed}, skipped {skipped}")
