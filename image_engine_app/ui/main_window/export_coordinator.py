...
class ExportCoordinator:
    ...
    def on_export_requested(self) -> None:
        ...
        kind = "fallback metadata" if getattr(result, "is_stub", False) else "real file"
        msg = getattr(result, "message", "Export complete")

        moved_next = False
        if self._window.export_bar.auto_next_after_export():
            moved_next = self.activate_next_asset_after_export()
    ...
    def activate_next_asset_after_export(self) -> bool:
