...
from app.settings_store import load_web_sources_settings, save_web_sources_settings
...
from engine.ingest.url_ingest import DownloadGuards
from engine.ingest.webpage_scan import WebpageScanCancelledError
...
class WebSourcesCoordinator:
    ...
    def on_download_requested(self, payload: object) -> None:
        ...
        if report.assets:
            self._window._register_assets(list(report.assets), set_active=True)
    ...
    def format_download_status(report: object) -> str:
        ...
        loaded = len(tuple(getattr(report, "assets", ()) or ()))
