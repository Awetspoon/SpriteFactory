...
from app.web_sources_models import Confidence, ImportTarget, ScanResults, SmartOptions, WebItem
...
class WebSourcesPanel(QFrame):
    ...
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        self._auto_sort = QCheckBox("Auto-sort by type", self)
        ...
        self._default_source_urls = self._default_area_urls_by_source_id()
    ...
    def smart_options(self) -> SmartOptions:
        return SmartOptions(
            ...
            auto_sort=bool(self._auto_sort.isChecked()),
    ...
    def set_smart_options(self, options: SmartOptions | dict | None) -> None:
        ...
        if isinstance(options, SmartOptions):
            ...
            smart = SmartOptions(
                ...
                auto_sort=bool(options.get("auto_sort", False)),
        ...
        self._auto_sort.setChecked(smart.auto_sort)
    ...
    def _build_ui(self) -> None:
        ...
        self.setStyleSheet("QFrame { border:1px solid #bfd2d4; border-radius:10px; background:#ffffff; }")
        ...
        self._auto_sort.setChecked(False)
        self._skip_dupes.setChecked(True)
        self._allow_zip.setChecked(True)
        opt.addWidget(self._show_likely)
        self._auto_sort.setToolTip("When enabled, GIF files go to Animated and name keywords route to Variant/Items automatically.")
        opt.addWidget(self._auto_sort)
        ...
        self._preview.setStyleSheet("QLabel { padding:8px; border:1px solid #d4e0e2; border-radius:8px; background:#f2f7f7; }")
        ...
        self._target.addItem("Main", ImportTarget.NORMAL)
        self._target.addItem("Variant", ImportTarget.SHINY)
        self._target.addItem("Animated", ImportTarget.ANIMATED)
        self._target.addItem("Items", ImportTarget.ITEMS)

        bottom.addWidget(QLabel("Save to:", self))
        bottom.addWidget(self._target)
        ...
        self._status.setStyleSheet("color:#4f6b70;")
    ...
    def _emit_download(self) -> None:
        ...
        if isinstance(target, ImportTarget):
    ...
    @staticmethod
    def _default_area_urls_by_source_id() -> dict[str, set[str]]:
        mapping: dict[str, set[str]] = {}
        for source in default_web_sources_registry():
            if not isinstance(source, dict):
                continue
            source_id = str(source.get("id", "")).strip()
            areas = source.get("areas")
            if not source_id or not isinstance(areas, list):
                continue
            urls = {
                str(area.get("url", "")).strip()
                for area in areas
                if isinstance(area, dict) and str(area.get("url", "")).strip()
            }
            if urls:
                mapping[source_id] = urls
        return mapping

    def _show_website_context_menu(self, pos) -> None:  # noqa: ANN001
        menu = QMenu(self)
        remove_action = menu.addAction("Remove Website")
        removable = self._is_selected_website_removable()
        if not removable:
            remove_action.setText("Remove Website (custom only)")
            remove_action.setEnabled(False)

        chosen = menu.exec(self._website.mapToGlobal(pos))
        if chosen is remove_action and removable:
    ...
    def _show_area_context_menu(self, pos) -> None:  # noqa: ANN001
        ...
        removable = self._is_selected_area_removable()
        if not removable:
            remove_action.setText("Remove URL (custom only)")
            remove_action.setEnabled(False)
    ...
    def _is_selected_website_removable(self) -> bool:
        ...
        if not source_id:
            return False
        return source_id not in self._default_source_urls

    def _is_selected_area_removable(self) -> bool:
        source = self._website.currentData()
        area = self._area.currentData()
        ...
        if not source_id or not area_url:
            return False

        default_urls = self._default_source_urls.get(source_id)
        if default_urls is None:
            return True
        return area_url not in default_urls
    ...
    def _remove_selected_website(self) -> None:
        ...
        if not self._is_selected_website_removable():
            self.set_status("Built-in websites cannot be removed.")
            return
    ...
    def _remove_selected_area(self) -> None:
        ...
        if not self._is_selected_area_removable():
            self.set_status("Built-in URLs cannot be removed.")
            return
