...
class PresetManagerDialog(QDialog):
    ...
    def __init__(self, controller, parent: QWidget | None = None) -> None:
        ...
        self._btn_example = QPushButton("Insert Example", self)
        self._btn_save = QPushButton("Save User Preset", self)
    ...
    def refresh_from_controller(self) -> None:
        ...
        for entry in self._controller.available_preset_entries():
    ...
    def _build_ui(self) -> None:
        ...
        intro = QLabel(
            "System presets are safe starting templates. Duplicate one or write your own user preset with scoped formats, tags, and a JSON settings delta.",
            header_card,
        )
        ...
        right_layout.addWidget(self._preset_kind)
        ...
        json_tools.addWidget(self._btn_example)
        json_tools.addStretch(1)
        right_layout.addLayout(json_tools)
    ...
    def _wire(self) -> None:
        ...
        self._btn_example.clicked.connect(self._insert_example_delta)
    ...
    def _format_delta_json(self) -> None:
        ...
        try:
            parsed = json.loads(raw_delta)
        except Exception as exc:
            QMessageBox.critical(self, "Format failed", f"Settings delta must be valid JSON before formatting:\n{exc}")
            return
        self._delta.setPlainText(json.dumps(parsed, indent=2, sort_keys=True))

    def _insert_example_delta(self) -> None:
        example = {
            "cleanup": {"denoise": 0.2, "artifact_removal": 0.15},
            "detail": {"sharpen_amount": 0.4},
            "export": {"format": "png"},
        }
        self._delta.setPlainText(json.dumps(example, indent=2, sort_keys=True))
    ...
    def _load_selected_into_form(self) -> None:
        ...
        self._refresh_editor_meta(selected, preset)
    ...
    def _load_draft_into_form(self, draft: _PresetDraft) -> None:
        ...
        self._scope_hint.setText(
            "Formats and tags are comma-separated scopes. Leave them blank for any asset. Examples: png, webp, gif | sprite_sheet, photo, icon, animation."
        )
        self._save_hint.setText(
            "Save creates a user preset. Duplicate a system preset first if you want a safer starting point."
        )

    def _refresh_editor_meta(self, preset_name: str, preset: PresetModel) -> None:
        is_user = bool(self._controller.is_user_preset(preset_name))
        scope_parts: list[str] = []
        formats = ", ".join(preset.applies_to_formats or ["*"])
        tags = ", ".join(preset.applies_to_tags or ["*"])
        scope_parts.append(f"Formats: {formats}")
        scope_parts.append(f"Tags: {tags}")
        self._scope_hint.setText(" | ".join(scope_parts))
        if is_user:
            self._preset_kind.setText("User preset")
            self._save_hint.setText("Saving updates this user preset in your local preset library.")
        else:
            self._preset_kind.setText("System template")
            self._save_hint.setText(
                "Saving with the same name creates a user override. Duplicate it first if you want to keep the system preset untouched."
            )
